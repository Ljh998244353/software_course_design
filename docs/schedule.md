...
- S00-S24 已完成。
- S25-S30 已规划但未开始。
- 当前下一步是 S25：在 S24 已有评价、通知中心和用户信用展示之上，接入统计报表和运维异常页面。
...
5. docs/测试计划与用例说明.md
6. docs/前端演示模块说明.md
7. 若继续 S25，还必须读取 docs/统计分析与报表模块说明.md、docs/系统监控与异常处理模块说明.md、src/modules/statistics/、src/modules/ops/、src/repository/statistics_repository.*、src/repository/ops_repository.*、tests/statistics/、tests/ops/、assets/app/、src/access/http/、scripts/test_http.sh。
...
- `/app` 真实业务前端骨架，已接入认证、卖家拍品、管理员审核、管理端拍卖、公开拍卖、真实出价、轮询通知、订单支付、履约、评价、通知中心和用户信用页面。

当前真实状态：

- `S00-S16`：课程设计原计划内步骤已完成。
- `S17-S24`：完整前端接入计划的前半段已完成。
- `S25-S30`：完整前端剩余计划未开始。
- 当前下一步：`S25`，接入统计报表和运维异常页面。
...
仍待接入的真实业务 HTTP 和页面范围：

- 统计报表、运维异常工作台。
- 完整前端 UX、页面级验收、部署脚本和最终 handoff。
...
- 结果：`auction_http_baseline` 通过，覆盖订单评价空列表、双方互评、重复评价拒绝、信用摘要、评价通知、通知已读、非参与者查询拒绝和订单状态推进到 `REVIEWED`；全量 `ctest` 为 `17/17` 通过。
...
| S25 | 未开始 | 统计报表和运维异常页面 | `src/access/http/statistics_http.*`、`src/access/http/ops_http.*`、`assets/app/`、`tests/http/` | `docs/统计分析与报表模块说明.md`、`docs/系统监控与异常处理模块说明.md` |
| S26 | 未开始 | 全站 UX、状态处理和安全前端约束 | `assets/app/`、`tests/http/`、`scripts/` | `docs/前端演示模块说明.md`、`docs/测试计划与用例说明.md` |
| S27 | 未开始 | 真实 HTTP 接口自动化回归收口 | `tests/http/`、`scripts/test_http.sh`、`scripts/test.sh`、`CMakeLists.txt` | `docs/测试计划与用例说明.md`、`docs/接口联调记录.md` |
| S28 | 未开始 | 前端页面级验收脚本 | `scripts/test_ui.sh`、`tests/ui/`、`assets/app/` | `docs/测试计划与用例说明.md`、`docs/前端演示模块说明.md` |
| S29 | 未开始 | 完整前端部署、演示数据和答辩脚本接入 | `scripts/deploy/`、`config/`、`sql/demo_data.sql`、`assets/app/` | `docs/部署与答辩说明.md`、`docs/答辩演示操作手册.md` |
| S30 | 未开始 | 完整前端最终回归和 handoff | 全仓库 | `docs/schedule.md`、`docs/部署与答辩说明.md`、`docs/答辩演示操作手册.md` |

## 6. 当前任务：S25

### 6.1 S25 目标

在 S24 已经完成评价、通知中心和用户信用展示的基础上，接入统计报表和运维异常页面，让管理员或客服能在 `/app` 中查看成交统计、关键业务日志、任务日志和异常处理入口。
...
- `src/modules/ops/`
- `src/repository/statistics_repository.*`
- `src/repository/ops_repository.*`
- `tests/statistics/`
- `tests/ops/`
- `assets/app/`
- `src/access/http/`
...
S25 必须复用 `StatisticsService`、`OpsService` 和既有仓储查询，不在 HTTP 层重写统计口径、异常判定或补偿规则。

### 6.4 S25 风险点

- 统计报表只能基于 MySQL 事实数据，不以缓存或前端聚合为准。
- 统计重建要保持幂等，重复执行不能重复累计。
- 运维日志和异常页面必须限制管理员或客服访问，普通用户不能读取后台日志。
- 补偿入口只能触发已有服务层安全动作，不能绕过订单、支付、通知状态机。
- 页面展示应保留最小筛选和错误反馈，避免把大量日志无约束拉到前端。
...
- Step ID: S24
- 当前状态: 已完成
- 已完成: 评价真实 HTTP 控制器、订单详情互评和信用摘要、独立通知中心、HTTP 回归主链路和负向场景、相关模块文档同步。
- 修改文件: `src/access/http/review_http.*`、`src/access/http/notification_http.*`、`src/access/http/business_http.cpp`、`CMakeLists.txt`、`assets/app/*`、`scripts/test_http.sh`、相关 `docs/*.md`。
- 验证命令: `cmake -S . -B build -DCMAKE_CXX_COMPILER=/usr/bin/c++ -DCRYPT_LIBRARY=/usr/lib/x86_64-linux-gnu/libcrypt.so`、`cmake --build build`、`bash -n scripts/test_http.sh`、`node --check assets/app/app.js`、`scripts/test_http.sh`、`ctest --test-dir build --output-on-failure`。
- 验证结果: 构建、脚本语法、`auction_http_baseline` 和全量 `ctest 17/17` 通过；HTTP/CTest 依赖本地测试 MySQL，已按权限流程在沙箱外通过。
- 未覆盖风险: S24 未新增页面级浏览器自动化截图验证；完整 UI 验收仍留到 S28。
- 下一步: S25 统计报表和运维异常页面。
- 下一步必须先读: `docs/统计分析与报表模块说明.md`、`docs/系统监控与异常处理模块说明.md`、`src/modules/statistics/`、`src/modules/ops/`、`tests/statistics/`、`tests/ops/`、`assets/app/`、`src/access/http/`、`scripts/test_http.sh`。
