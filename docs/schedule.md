...
- S00-S27 已完成。
- S28-S30 已规划但未开始。
- 当前下一步是 S28：在 S27 已完成真实 HTTP 接口自动化回归收口后，补齐前端页面级验收脚本。
...
5. docs/测试计划与用例说明.md
6. docs/前端演示模块说明.md
7. 若继续 S28，还必须读取 docs/测试计划与用例说明.md、docs/前端演示模块说明.md、assets/app/、scripts/test_http.sh、scripts/test.sh、CMakeLists.txt。
...
- 结果：构建、脚本语法、空白检查、`auction_http_baseline`、`scripts/test.sh http` 和全量 `ctest 17/17` 通过；HTTP 基线覆盖 S17-S26 原有真实主链路和静态前端约束，并新增支付坏签名回调拒绝且订单/支付状态不推进断言、阶段输出、运行前临时响应清理、失败时响应文件和服务日志诊断。
...
在 S27 已经完成真实 HTTP 接口自动化回归收口的基础上，补齐 `/app` 前端页面级验收脚本，让关键页面结构、路由入口、角色可见性和基础交互约束可以在最终交付前自动复核。
...
- 在不引入前端构建链的前提下，建立轻量页面级验收入口。
- 验证 `/app` 的关键视图、表单约束、角色入口、通知/订单/统计/运维页面结构和窄屏基础布局约束。
- 保持 `scripts/test.sh http` 和全量 `ctest` 仍可执行，不弱化 S27 已有真实 HTTP 主链路。
- 将页面级验收范围同步到测试计划和前端演示模块说明。

S28 不新增业务功能；只有发现真实前端页面结构、路由入口或测试基线缺陷时才做最小修复。

### 6.4 S28 风险点

- 不要为了页面测试引入 Node/Vite/React 构建链。
- 页面级验收不能替代 S27 真实 HTTP 主链路回归。
- 不要为测试便利放宽前端输入约束、角色入口或后端 RBAC。
- 不要引入付费服务或前端构建链。
...
- Step ID: S27
- 当前状态: 已完成
- 已完成: 真实 HTTP 回归脚本运行前响应文件清理、分阶段输出、失败时阶段/行号/命令/最近响应/服务日志诊断、支付坏签名回调拒绝且订单/支付状态不推进断言、`auction_http_baseline` CTest 600 秒超时。
- 修改文件: `scripts/test_http.sh`、`CMakeLists.txt`、`docs/schedule.md`、`docs/测试计划与用例说明.md`、`docs/接口联调记录.md`。
- 验证命令: `cmake --build build`、`bash -n scripts/test_http.sh`、`git diff --check`、`scripts/test_http.sh`、`scripts/test.sh http`、`ctest --test-dir build --output-on-failure`。
- 验证结果: 构建、脚本语法、空白检查、`auction_http_baseline`、`scripts/test.sh http` 和全量 `ctest 17/17` 通过；验证前仅清理了一个指向本项目 `build/test_mysql/data` 且 socket 已丢失的残留测试 `mysqld`。
- 未覆盖风险: S27 未新增浏览器级截图或真实交互自动化；页面级验收仍留到 S28。
- 下一步: S28 前端页面级验收脚本。
- 下一步必须先读: `docs/测试计划与用例说明.md`、`docs/前端演示模块说明.md`、`assets/app/`、`scripts/test_http.sh`、`scripts/test.sh`、`CMakeLists.txt`。
