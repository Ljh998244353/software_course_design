...
actor 卖家 as Seller
actor 管理员 as Admin
actor "运维/客服" as Support
rectangle "在线拍卖平台" as System
rectangle "第三方支付服务" as Pay
rectangle "图片或对象存储" as Storage
rectangle "邮件或短信服务" as Notify

Buyer --> System : 浏览活动/出价/支付/评价
Seller --> System : 发布物品/查看审核/确认交易/评价
Admin --> System : 审核物品/配置活动/查看报表/处理异常
Support --> System : 查看日志/协助处理纠纷

System --> Pay : 支付下单/支付查询
Pay --> System : 支付回调
System --> Storage : 上传图片/读取图片
System --> Notify : 发送邮件或短信
@enduml
```
...
actor "浏览器前台 / 管理后台" as Client
node "Nginx 反向代理" as Nginx

package "在线拍卖平台应用" as App {
    package "接入层" as Access {
        component "REST API" as Rest
        component "WebSocket Gateway" as Ws
        component "管理接口" as AdminApi
    }

    package "应用层" as Application {
        component "认证与权限应用服务" as AuthApp
        component "物品与审核应用服务" as ItemApp
        component "拍卖管理应用服务" as AuctionApp
        component "竞价应用服务" as BidApp
        component "订单应用服务" as OrderApp
        component "支付应用服务" as PaymentApp
        component "评价应用服务" as ReviewApp
        component "统计应用服务" as StatsApp
        component "通知应用服务" as NotifyApp
        component "调度与补偿任务" as JobApp
    }

    package "领域层" as Domain {
        component "用户域" as UserDomain
        component "物品域" as ItemDomain
        component "拍卖域" as AuctionDomain
        component "竞价域" as BidDomain
        component "订单域" as OrderDomain
        component "支付域" as PaymentDomain
        component "评价域" as ReviewDomain
    }

    package "基础设施层" as Infra {
        component "MySQL Repository" as Repo
        component "Redis Cache" as Cache
        component "文件存储适配器" as FileAdapter
        component "支付适配器" as PayAdapter
        component "日志与审计" as LogAudit
    }
}
...
actor 卖家 as Seller
actor 管理员 as Admin
participant "物品接口" as ItemApi
participant "物品与审核服务" as ItemSvc
database "MySQL" as DB
collections "文件存储" as Storage
participant "通知服务" as Notify

Seller -> ItemApi : 提交物品信息与图片
ItemApi -> ItemSvc : 保存拍品
ItemSvc -> Storage : 存储图片
ItemSvc -> DB : 写入物品与图片元数据
Admin -> ItemApi : 审核拍品
ItemApi -> ItemSvc : 提交审核请求
ItemSvc -> DB : 写审核结果与日志
ItemSvc -> Notify : 发送审核结果通知
Notify -> Seller : 审核结果通知
@enduml
```
...
@startuml
actor 买家 as Buyer
participant "出价接口" as BidApi
participant "鉴权" as Auth
participant "竞价服务" as BidSvc
...
BidApi -> Auth : 校验身份
Auth -> BidSvc : 调用出价能力
BidSvc -> Cache : 读取热点活动快照
BidSvc -> DB : 事务校验并更新
BidSvc -> Cache : 刷新缓存
BidSvc -> Notify : 发送价格变更事件
Notify -> Buyer : 推送最新价格
@enduml
```
...
```plantuml
@startuml
participant "调度任务" as Job
participant "拍卖服务" as AuctionSvc
participant "订单服务" as OrderSvc
participant "支付服务" as PaymentSvc
database "MySQL" as DB
participant "第三方支付" as Pay
participant "通知服务" as Notify

Job -> AuctionSvc : 扫描到期活动
AuctionSvc -> DB : 关闭活动并判定结果
AuctionSvc -> OrderSvc : 触发生成订单
OrderSvc -> DB : 生成待支付订单
OrderSvc -> Notify : 发送待支付通知
PaymentSvc -> Pay : 发起支付
Pay -> PaymentSvc : 支付回调
PaymentSvc -> DB : 幂等更新支付和订单状态
PaymentSvc -> Notify : 发送支付结果通知
@enduml
```
...
node "浏览器" as Client
node "Nginx" as Nginx
node "在线拍卖平台 C++ 应用" as App
database "MySQL" as MySQL
database "Redis" as Redis
node "本地文件或对象存储" as Storage
cloud "第三方支付" as Pay
...
以下排期以 8 周课程设计周期为例，目标是在单机或实验室环境下完成“环境可复现、主链路闭环、测试可验证、文档可答辩”的交付要求。

#### 7.4.1 环境与工程基线

建议在第 1 周内统一以下默认开发与运行基线：

- 开发环境：WSL2 Ubuntu 24.04。
- 编译工具链：C++20、GCC 或 Clang、CMake、Git。
- 后端基础组件：Drogon、MySQL 8、Redis 7、OpenSSL。
- 部署与接入：Nginx 作为反向代理与静态资源入口。
- 测试工具：GoogleTest、CTest，必要时补充并发压测与接口测试工具。
- 代码目录：`src/`、`tests/`、`assets/`，并同步建立统一配置、日志、错误码和脚本目录。

#### 7.4.2 周度 Schedule

| 周次 | 环境配置与工程任务 | 模块开发重点 | 测试与验收重点 | 阶段输出 |
|---|---|---|---|---|
| 第 1 周 | 完成编译器、CMake、Drogon、MySQL、Redis、Nginx、本地配置文件与数据库初始化脚本；搭建基础工程骨架与统一响应模型 | 认证与权限模块：用户模型、注册登录、Token 鉴权、RBAC 基础能力 | 环境冒烟测试、编译测试、数据库连通测试、认证单元测试、鉴权失败路径测试 | 可编译工程骨架、环境配置清单、认证接口草案 |
| 第 2 周 | 补齐文件上传、图片访问、种子数据和基础管理端路由 | 物品与审核模块：拍品发布、图片管理、审核流转、审核日志 | 字段校验测试、上传失败测试、审核权限测试、物品状态流转测试 | 物品与审核模块说明、建表脚本初稿、基础前后台联调结果 |
| 第 3 周 | 增加调度任务框架、时间处理工具和活动配置页面支撑 | 拍卖管理模块：拍卖创建、时间规则、状态切换、活动查询 | 拍卖状态机测试、时间边界测试、活动创建集成测试、非法状态转换测试 | 拍卖管理模块说明、活动接口定义、调度任务骨架 |
| 第 4 周 | 完成 Redis 热点缓存、WebSocket 通道、关键事务日志能力 | 竞价与实时通知模块：出价校验、最高价更新、延时保护、历史记录、价格推送 | 并发出价测试、过期价格测试、`end_time` 边界测试、延时保护测试、通知失败不回滚测试 | 竞价模块说明、竞价事务方案、并发测试样例 |
| 第 5 周 | 接入支付适配器桩实现、订单超时任务、回调验签基础能力 | 订单与支付结算模块：订单生成、支付发起、支付回调、幂等控制、超时关闭 | 单拍卖单订单测试、重复回调测试、金额不一致测试、回调验签失败测试、审计日志测试 | 订单/支付模块说明、支付接口契约、幂等处理方案 |
| 第 6 周 | 增加统计任务、消息重试、日志查询与异常处理入口 | 评价模块、通知模块、统计模块、运维与异常处理模块 | 评价权限测试、统计重跑测试、Redis 降级测试、任务重试测试、通知补偿测试 | 外围模块说明、统计口径定义、异常处理与补偿说明 |
| 第 7 周 | 冻结主要配置，开始系统联调、性能调优和安全加固 | 全模块联调，重点修正竞价、结算、支付回调、权限控制中的集成问题 | 端到端主流程测试、安全负向测试、接口契约测试、200 并发在线验证、出价接口平均响应与 P95 验证 | 联调问题清单、性能测试记录、安全问题修复结果 |
| 第 8 周 | 固化部署脚本、演示数据、备份恢复脚本和答辩环境 | 缺陷收敛、文档补齐、演示流程整理、最终发布包准备 | 全量回归测试、重复运行稳定性检查、关键链路复测、答辩脚本验收 | 最终版本代码、测试报告、部署说明、答辩演示材料 |

#### 7.4.3 测试推进节奏

测试工作不应集中到最后一周，建议随模块开发同步推进：

1. 第 1 至 3 周以单元测试、参数校验测试、状态机测试和基础集成测试为主。
2. 第 4 至 5 周重点覆盖竞价并发、拍卖结束、订单生成、支付回调幂等这三条高风险链路。
3. 第 6 周补齐降级、补偿、重试、日志审计和统计任务相关测试。
4. 第 7 至 8 周执行端到端测试、性能测试、安全负向测试和全量回归测试。

建议在排期执行过程中设置以下阶段门禁：

- 未完成环境冒烟测试和基础编译链路前，不进入核心模块编码。
- 未完成竞价并发测试、拍卖结束测试和支付回调幂等测试前，不进入最终联调收尾阶段。
- 每完成一个模块，都应同步更新 `docs/` 下对应的 Markdown 说明，至少包含功能职责、接口定义、输入输出、错误场景和状态流转。
